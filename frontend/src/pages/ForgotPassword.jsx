...
import { Mail, ArrowLeft, ShoppingBag, ShieldCheck } from 'lucide-react';
      ...
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col lg:flex-row">
        <div className="hidden lg:flex lg:w-1/2 flex-col justify-center px-12 py-16 border-r border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900/60">
          <div className="max-w-md">
            ...
            <div className="mt-8 space-y-3 text-sm text-stone-600 dark:text-stone-400">
              <div className="flex items-center gap-3"><ShieldCheck className="h-4 w-4 text-emerald-600" />Secure password reset</div>
              <div className="flex items-center gap-3"><Mail className="h-4 w-4 text-emerald-600" />Link expires in 1 hour</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-12">
          ...
          >
            ...
            <div className="mb-8">
              <h2 className="text-3xl font-semibold text-stone-900 dark:text-white">Forgot password</h2>
              <p className="mt-2 text-stone-600 dark:text-stone-400">Enter email to receive a reset link.</p>
            </div>

            {sent ? (
              <div className="rounded-3xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 p-8 shadow-sm text-center">
                ...
                <h3 className="text-xl font-semibold text-stone-900 dark:text-white mb-2">Check your email</h3>
                <p className="text-stone-600 dark:text-stone-400 mb-6">
                  If an account exists for <strong className="text-stone-900 dark:text-white">{email}</strong>, we've sent a password reset link.
                </p>
                <Link
                  to="/login"
                  className="inline-flex items-center gap-2 text-sm font-medium text-stone-900 hover:underline dark:text-white"
                >
              ...
              <form onSubmit={submitHandler} className="space-y-5 rounded-3xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 p-6 shadow-sm">
                ...
                >
                  {isLoading ? <div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent dark:border-stone-950 dark:border-t-transparent" /> : 'Send Reset Link'}
                </motion.button>
              </form>
            )}
